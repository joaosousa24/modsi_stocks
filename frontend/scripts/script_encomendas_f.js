async function fetchEncomendas() {
    try{
     ...
     const data = await response.json();
       ...
       const tableBody = document.getElementById('encomendasFronecedor').getElementsByTagName('tbody')[0];
       tableBody.innerHTML = '';
  
       // Preencher a tabela com os dados recebidos
       data.enc.forEach((enc) => {
           const row = tableBody.insertRow();
           row.insertCell(0).innerText = enc.Nome_F;
           row.insertCell(1).innerText = enc.ID_Pedido;
           row.insertCell(2).innerText = new Date(enc.data).toLocaleString();
  ...
  fetchEncomendas();
    ...
    async function create_div(id_ped){
    ...
    const enc_p_info = await getEncomendaInfo(id_ped);
    
    enc_p_info.enc.forEach(async (enc) => {
       
    const pord_name= await getprod_name(enc.produto);
  ...
  pord_name.prod.forEach(async (prod) =>{
    ...
    async function getEncomendaInfo(id_ped) {
        try {
            const response = await fetch('/encomendas_data/fornecedores', {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
                },
                body: JSON.stringify({res: id_ped }),
            });
